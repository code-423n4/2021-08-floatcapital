...
contract YieldManagerAave is IYieldManager {
  ...
  */
  ...
  function transferPaymentTokensToUser(address user, uint256 amount) external override longShortOnly {
    ...
    ILendingPool(lendingPoolAddressesProvider.getLendingPool()).withdraw(address(paymentToken), amount, user);
  }
  ...
  function removePaymentTokenFromMarket(uint256 amount) external override longShortOnly {
    try
      ILendingPool(lendingPoolAddressesProvider.getLendingPool()).withdraw(address(paymentToken), amount, address(this))
    {} catch {
  ...
  */
  function claimAaveRewardsToTreasury() external {
    IAaveIncentivesController _aaveIncentivesController = IAaveIncentivesController(aaveIncentivesController);
